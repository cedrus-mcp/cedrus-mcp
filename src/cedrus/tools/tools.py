...
See Also:
    - cedrus.tools.tool_registry: Registry infrastructure for tool variants
    - cedrus.server: Server initialization and startup tool registration
    - cedrus.models.base: Mode type definition
"""
...
from cedrus.tools import relation_elaborating, suggestions
from cedrus.tools import node_creation, node_updates, node_deletion
from cedrus.tools.tool_args import parse_tool_args
from cedrus.tools.tool_context import tool_context
...
async def connect_elaborate(
    ...
    relation_type: DialecticalRelationType = "support",
    target_premise_idx: int | None = None,
    grounding_strategy: Literal['define_negation', 'define_equivalence', 'copy_premise', 'negate_premise', 'copy_conclusion', 'negate_conclusion', "none"] = "none",
) -> CallToolResult:
    ...
    return await _connect_impl(
        ...
        relation_type=relation_type,
        target_premise_idx=target_premise_idx,
        grounding_strategy=grounding_strategy if grounding_strategy != "none" else None
    )
...
#         try:
#             svg_string = cedrus.graph.svg_export.export_svg(arg_map)
