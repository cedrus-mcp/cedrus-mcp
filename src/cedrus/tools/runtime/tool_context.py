...
class ToolContext:
    ...
    def suggest(
        self, tool: ToolName, params: dict[str, Any], reason: str, action_type: str = "expand"
    ) -> ToolContext:
    ...
    def note(
        self, text: str, priority: float = 0.3, audience: list[Role] | None = None
    ) -> ToolContext:
        ...
        self.content.append(
            TextContent(
                type="text",
                text=text,
                annotations=Annotations(audience=audience or default_audience, priority=priority),
            )
        )
        return self

    def embed_resource(
        self, uri: AnyUrl, text: str, priority: float = 0.5, audience: list[Role] | None = None
    ) -> ToolContext:
    ...
    def build(self) -> CallToolResult:
        ...
        if self._status == "failure":
            ...
            return CallToolResult(content=self.content, structuredContent=structuredContent)
        ...
        if self.resources:
            ...
            text = "\n\n".join(
                ...
                if isinstance(r.resource, TextResourceContents)
            )
        ...
        if self.suggestions:
            ...
            text = "\n\n".join(
                f"{s.action_type.upper()} {s.reason.strip(' .')}: `{s.tool}({s.params if s.params else ''})`"
    ...
    def suggest_support_argument(
        self, label: str, reason: str, target_premise_idx: int | str | None = None
    ) -> ToolContext:
        ...
        params: dict[str, Any] = {"target": label, "source": new_label, "relation_type": "support"}
        if target_premise_idx is not None:
            params["target_premise_idx"] = target_premise_idx

        self.suggest(
            "add_argument",
            {"label": new_label, "gist": f"Key point of new argument <{new_label}> goes here"},
            f"{reason} (Step 1 of 2)",
            action_type="expand",
        )
        self.suggest("connect", params, f"{reason} (Step 2 of 2)", action_type="expand")
        return self
    ...
    def suggest_update_field(
        ...
        return self.suggest(
            "edit",
            {
                "label": label,
                "field": field,
                "edit_options": {"new_value": f"{field.upper()}_VALUE"},
            },
            reason,
            action_type="improve",
        )
...
@contextmanager
def tool_context(arg_map: ArgumentMap, mode: Mode = "sketch") -> Iterator[ToolContext]:
