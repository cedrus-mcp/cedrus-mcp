...
def add_suggestions_after_adding_node(
    ...
    if claim_added := arg_map.get_claim(label):
        ...
        if tc.mode in ["sketch", "elaborate"]:
            ...
            if (
                not arg_map.get_supported(label)
                and not arg_map.get_attacked(label)
                and len(arg_map.list_arguments()) > 1
            ):
                ...
                tc.suggest(
                    "connect",
                    {"source": label, "target": "Existing-Argument-Label", **relation_options},
                    reason,
                    action_type="connect",
                )
    ...
    ## ARGUMENT NODE has been added ##
        ...
        if any(issue.severity in ["warning", "error"] for issue in tc.issues) and tc.mode in [
            "sketch",
            "elaborate",
        ]:
            return
        ...
        if tc.mode == "sketch":
            ...
            if (
                not arg_map.get_attacked(label)
                and not arg_map.get_supported(label)
                and len(arg_map.list_node_labels()) > 1
            ):
                tc.suggest(
                    "connect",
                    {
            ...
            if not arg_map.get_supporters(label):
                tc.suggest_support_argument(
                    label=label, reason=f"Add a further argument which supports <{label}>."
                )
            if not arg_map.get_attackers(label):
                tc.suggest_attack_argument(
                    label=label, reason=f"Add a further argument which attacks <{label}>."
                )
        ...
        if tc.mode == "elaborate":
            ...
            if not argument_added.conclusion:
                tc.suggest_update_field(
                    label=argument_added.label,
                    field="conclusion",
                    reason="State explicitly the conclusion of this argument.",
                )
                tc.suggest(
                    "connect",
                    {
            ...
            if not argument_added.premises or len(argument_added.premises) < 2:
                tc.suggest(
                    "edit",
                    {
                        "label": label,
                        "field": "premises",
                        "edit_options": {"premise_idx": 1, "new_value": "PREMISE_TEXT"},
                    },
