...
def deliberate_decision(decision_problem: str) -> str:
    ...
    return (
        ...
        "When using reasoning tools, start with `sketch` mode (possibly resetting the graph) to capture the overall "
        "argumentation, and switch to `elaborate` mode for adding detailed content and refining the preliminary draft. "
        "Then, iterate between `review` and `elaborate` mode to consolidate the reasoning representation until it passes "
        "all validation checks and meets your own expectations.\n"
