...
def test_validate_empty_map_author_mode(empty_map: ArgumentMap, author_context: ToolContext) -> None:
    """Test validation of empty map in author mode runs without errors."""
    issues = validate_argument_map(empty_map, author_context)
    assert issues == 0
...
def test_validate_simple_claim_author_mode(simple_claim_map: ArgumentMap, author_context: ToolContext) -> None:
    """Test validation of simple claim in author mode."""
    author_context.arg_map = simple_claim_map
    issues = validate_argument_map(simple_claim_map, author_context)
    assert issues == 0


def test_validate_returns_issue_count(empty_map: ArgumentMap, author_context: ToolContext) -> None:
...
def test_validate_with_fix_parameter(empty_map: ArgumentMap, author_context: ToolContext) -> None:
    ...
    # Test without fix
    issues_without_fix = validate_argument_map(empty_map, author_context, fix=False)
    
    # Reset context
    author_context = ToolContext(arg_map=empty_map, mode="author")
    
    # Test with fix - should also run without error
    issues_with_fix = validate_argument_map(empty_map, author_context, fix=True)
...
def test_validate_with_max_issues_parameter(simple_claim_map: ArgumentMap, author_context: ToolContext) -> None:
    """Test that max_issues parameter limits reported issues."""
    author_context.arg_map = simple_claim_map
    # Validation should complete with max_issues set
    issues = validate_argument_map(simple_claim_map, author_context, max_issues=5)
    assert issues <= 5
...
def test_validate_mode_specific_checks_author(empty_map: ArgumentMap, author_context: ToolContext) -> None:
    """Test that author mode runs all validation checks."""
    # Add incomplete argument node
    prop_c = Proposition(content="Conclusion")
    empty_map.add_proposition(prop_c)
    ...
    # In author mode, should run all checks including argument structure
    issues = validate_argument_map(empty_map, author_context)
...
def test_validate_pipeline_order(empty_map: ArgumentMap, review_context: ToolContext) -> None:
    ...
    # 2. argument structure (author/review)
    # 3. grounding (author/review)
    # 4. completeness (author/review)
    # 5. consistency (author/review)
