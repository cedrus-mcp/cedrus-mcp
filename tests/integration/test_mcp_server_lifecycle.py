...
from cedrus.backend.graph.persistence import save_graph
from cedrus.backend.models import ClaimNode, Proposition
from cedrus.server import app_lifespan, mcp


@pytest.mark.asyncio
async def test_server_lifespan_creates_new_graph(
    temp_data_file: Path, monkeypatch: MonkeyPatch
) -> None:
...
@pytest.mark.asyncio
async def test_server_lifespan_loads_existing_graph(
    temp_data_file: Path, sample_arg_map: ArgumentMap, monkeypatch: MonkeyPatch
) -> None:
...
@pytest.mark.asyncio
async def test_server_lifespan_saves_on_shutdown(
    temp_data_file: Path, monkeypatch: MonkeyPatch
) -> None:
